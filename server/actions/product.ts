...
export const fetchProductByFullId = async (
  productId: string
): Promise<ProductsWithVariants | null> => {
  try {
    const product = await db.query.products.findFirst({
      where: eq(products.id, productId),
      with: {
...
export const fetchAllProducts = async (params: {
  ...
  try {
      ...
      filters.push(
        exists(
          db
            ...
            .innerJoin(
              productVariantOption,
              eq(productVariant.id, productVariantOption.productVariantId)
            )
            .where(
              and(
                eq(productVariant.productId, products.id),
                minPrice !== undefined
                  ? gte(productVariantOption.price, minPrice)
                  : undefined,
                maxPrice !== undefined
                  ? lte(productVariantOption.price, maxPrice)
                  : undefined
              )
            )
        )
      ...
      filters.push(
        exists(
          db
            ...
            .innerJoin(
              productVariantCondition,
              eq(productVariant.id, productVariantCondition.productVariantId)
            )
            .where(
              and(
                eq(productVariant.productId, products.id),
    ...
    const allProducts = await db.query.products.findMany({
      ...
      with: {
        favouriteProduct: {
          where: (favouriteProduct, { eq }) =>
            eq(favouriteProduct.userId!, currentUser?.id!),
        },
        productVariant: {
          with: {
            productVariantOption: true,
    ...
    if (sort === 'price-low' || sort === 'price-high') {
      allProducts.sort((a: any, b: any) => {
        const priceA =
          a.productVariant?.[0]?.productVariantOption?.[0]?.price || 0;
        const priceB =
          b.productVariant?.[0]?.productVariantOption?.[0]?.price || 0;
        return sort === 'price-low' ? priceA - priceB : priceB - priceA;
      });
    }

    // Fetch all active discounts once to avoid N+1 queries
    const now = new Date();
    ...
    const mappedProducts = allProducts.map((product) => {
      // Find a matching discount rules based on the base price of the first variant/option
      const price =
        product.productVariant?.[0]?.productVariantOption?.[0]?.price || 0;

      const applicableDiscount = activeDiscounts.find(
        (d) => price >= d.minPrice && price <= d.maxPrice
      );
      ...
      return {
        ...
        discount: applicableDiscount
          ? {
              ...
              finalPrice: price - (price * applicableDiscount.discount) / 100,
              startDate: applicableDiscount.startDate,
              endDate: applicableDiscount.endDate,
            }
          : null,
  ...
  try {
    const [favouriteProducts, total] = await Promise.all([
      db.query.favouriteProduct.findMany({
        ...
        orderBy: (favouriteProduct, { desc }) => [
          desc(favouriteProduct.createdAt),
        ],
      }),
      db
        .select({ value: count() })
    ...
    const mappedProducts = favouriteProducts.map((product) => {
      return {
        ...product,
        favouriteProduct:
          product.products.favouriteProduct.length > 0 ? true : false,
      };
    });
...
export const AddProductAction = actionClient
  ...
  .action(
    async ({
      parsedInput: { title, description, productId, category, brand },
    }) => {
      try {
    ...
    async ({
      ...
      try {
        ...
        if (editMode && id) {
          ...
          await db
            .delete(productVariantOption)
            .where(eq(productVariantOption.productVariantId, variant[0].id));
          ...
          await db
            ...
            .where(eq(productVariantImage.productVariantId, variant[0].id));
          ...
          await db
            ...
            .where(eq(productVariantColor.productVariantId, variant[0].id));
          ...
          await db
            .delete(productVariantCondition)
            .where(eq(productVariantCondition.productVariantId, variant[0].id));
